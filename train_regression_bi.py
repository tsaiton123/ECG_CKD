...
from CNN_BiLSTM import build_ecg_model
import pandas as pd
import numpy as np
from utils import *
...
def train(train_gen, val_gen, epochs, input_shape, output_classes, model_path, checkpoint, class_weight=None):
    ...
    with tf.device('/gpu:0'):
        ...
        model = build_ecg_model(input_shape, output_classes, last_layer='linear')
...
if __name__ == '__main__':
    ...
    # expand dimension of x into (n, 12, 1000, 1)
    new_train_x = np.expand_dims(new_train_x, axis=-1)
    new_val_x = np.expand_dims(new_val_x, axis=-1)
    ...
    train(train_gen, val_gen, num_epoch,(12, segment_length, 1), num_classes, 'model', checkpoint, class_weight=None)
